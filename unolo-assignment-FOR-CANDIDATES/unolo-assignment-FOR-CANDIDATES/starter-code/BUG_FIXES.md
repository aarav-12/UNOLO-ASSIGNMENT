...
## Bug 7: Dashboard Shows Incorrect Data for Some Users

**Location:** `frontend/src/pages/Dashboard.jsx` - Line 15

**What was wrong:**
The dashboard endpoint selection used `user.id === 1` to determine if the user is a manager. This hardcoded approach meant only the user with ID 1 would see the manager dashboard, regardless of their actual role.
...
**Why this fix is correct:**
User roles should be determined by the `role` field, not by hardcoded IDs. This ensures any user with the "manager" role gets the correct dashboard, regardless of their user ID.

---

## Bug 8: Check-in Form Doesn't Submit Properly

**Location:** `frontend/src/pages/CheckIn.jsx` - Line 84

**What was wrong:**
The `handleCheckIn` function was missing `e.preventDefault()`, causing the form to submit with the default browser behavior (page reload) instead of handling the submission via JavaScript/AJAX.
...
**Why this fix is correct:**
When handling form submissions in React with custom logic (like API calls), `e.preventDefault()` must be called to prevent the browser's default form submission behavior, which would cause a page reload and interrupt the async operation.

---

## Bug 9: React Component Performance Issues

### Bug 9a: History Page Crashes on Load
...
### Bug 9b: Counter Component Stale Closure
...
### Bug 9c: Conditional useEffect Hook
...
All 9 bugs have been identified and fixed:
...
- ✅ Dashboard incorrect data (hardcoded user ID)
- ✅ Check-in form submission (missing preventDefault)
